...
class Interface(object):
    ...
    def run(self, folder_names=None):
        """
        Run interface detection.

        Parameters
        ----------
        folder_names : list of str, optional
            If provided, only process these folder names (from the filtered
            candidate list). If None, processes all subdirs in pdb_dir.
        """
        pdb_dir = self.config.pdb_dir

        if folder_names is not None:
            # Only process folders that passed the score cutoff in Stage 1
            subdirs = []
            for name in sorted(folder_names):
                ...
                if d.is_dir():
                    ...
                    print("[Interface] Folder not found, skipping: {}".format(name), flush=True)
        else:
